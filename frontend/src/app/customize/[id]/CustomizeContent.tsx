...
export default function CustomizeContent({ product }: { product: DbProduct }) {
  ...
  const [sel, setSel]   = useState<Selections>({
    shapeId: "", chocolateId: "", chocolateColorId: "", toppingIds: [], writingText: "",
  });
  ...
  const canProceed =
    ...
    : step === 2 ? (selectedChoc?.hasColor ? !!sel.chocolateColorId : true)
    : true;
  ...
  const handleNext = () => {
    ...
    addToCart({
      ...
      customization: {
        ...
        flavor:   selectedChoc ? (chosenColor ? `${selectedChoc.name} — ${chosenColor.name}` : selectedChoc.name) : undefined,
          ...
          return (
            <button key={shape.id} onClick={() => setSel((p) => ({ ...p, shapeId: shape.id }))}
              className={cn("relative rounded-2xl border-2 p-3 text-left transition-all duration-300 bg-white active:scale-[0.97] shadow-warm-sm",
                selected ? "border-[#3D0A14] shadow-warm-md" : "border-[rgba(26,10,10,0.10)] hover:border-[#3D0A14]/40 hover:shadow-warm-sm")}>
              {selected && <CheckCircle2 size={18} className="absolute top-3 right-3 text-[#3D0A14] fill-[#F5E4E6]" />}
              ...
              <div className="pb-1">
                ...
                <p className={cn("font-bold text-xl leading-none", selected ? "text-[#3D0A14]" : "text-[#4A3728]")}>{shape.price} QAR</p>
              </div>
            </button>
          );
        })}
    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        {chocolates.map((choc) => {
          ...
          return (
            <button key={choc.id} onClick={() => setSel((p) => ({ ...p, chocolateId: choc.id, chocolateColorId: "" }))}
              className={cn("relative rounded-2xl border-2 p-4 text-center transition-all duration-300 bg-white active:scale-[0.97]",
                selected ? "border-[#3D0A14] shadow-warm-md" : "border-[rgba(26,10,10,0.10)] hover:border-[#3D0A14]/40 hover:shadow-warm-sm")}>
              {selected && <CheckCircle2 size={16} className="absolute top-2.5 right-2.5 text-[#3D0A14] fill-[#F5E4E6]" />}
    ...
    <div>
      ...
      <div className="flex gap-6">
        {whiteChocolateColors.map((color) => {
          ...
          return (
            <button key={color.id} onClick={() => setSel((p) => ({ ...p, chocolateColorId: color.id }))}
              disabled={!selectedChoc?.hasColor}
              className={cn("flex flex-col items-center gap-2", !selectedChoc?.hasColor && "opacity-45 cursor-not-allowed")}>
              <div className="relative">
                <div className={cn("w-16 h-16 rounded-full border-4 transition-all duration-200", selected ? "border-[#3D0A14] scale-110 shadow-warm-md" : "border-[rgba(26,10,10,0.12)]")} style={{ backgroundColor: color.hex }} />
                {selected && (
                  <span className="absolute inset-0 flex items-center justify-center">
                    <Check size={16} className={color.id === "white" ? "text-[#4A3728]" : "text-white"} strokeWidth={3} />
                  </span>
                )}
              </div>
          ...
          return (
            <button key={topping.id} onClick={() => toggleTopping(topping.id)}
              className={cn("relative rounded-2xl border-2 p-4 text-center transition-all duration-200 bg-white active:scale-[0.97]",
                selected ? "border-[#3D0A14] bg-[#F5E4E6] shadow-warm-sm" : "border-[rgba(26,10,10,0.10)] hover:border-[#3D0A14]/40")}>
              {selected && <span className="absolute top-2 right-2"><Check size={12} className="text-[#3D0A14]" strokeWidth={3} /></span>}
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: "#C896A0" }}>
      <header className="fixed top-0 left-0 right-0 z-50 bg-[#C896A0]/97 backdrop-blur-md border-b border-[rgba(26,10,10,0.10)] shadow-warm-xs h-14 flex items-center px-4 md:px-6">
        <button onClick={() => router.back()} className="flex items-center gap-1 text-[#9E7B7B] hover:text-[#3D0A14] transition-colors mr-3">
          <ChevronLeft size={22} />
        </button>
        <h1 className="font-playfair font-semibold text-[#1A0A0A] text-lg flex-1 truncate">{product.name}</h1>
        <div className="text-right">
          <p className="text-[10px] text-[#9E7B7B] uppercase tracking-wider leading-none mb-0.5">Total</p>
          <p className="font-playfair text-lg font-bold text-[#3D0A14] leading-none">QAR {totalPrice}</p>
        </div>
      </header>
      ...
      <div className="flex flex-1 pt-14 pb-20 bg-[#FDF6F0]">
          ...
          {steps.map((s, i) => {
            ...
            return (
              <button key={s.id} onClick={() => setStep(i)}
                className={cn("flex flex-col items-center gap-1.5 py-4 px-2 transition-all duration-300 relative", active ? "text-white" : completed ? "text-[#3D0A14]" : "text-[#9E7B7B]")}>
                {active && <span className="absolute inset-x-2 inset-y-1 bg-[#3D0A14] rounded-xl shadow-warm-sm" />}
                <span className="relative z-10">{completed ? <CheckCircle2 size={20} className="fill-[#F5E4E6]" /> : <s.Icon size={20} />}</span>
        ...
        <main className="flex-1 md:ml-20 flex flex-col">
              ...
              return (
                <button key={s.id} onClick={() => setStep(i)}
                  className={cn("flex-1 flex flex-col items-center gap-1 py-3 px-2 border-b-2 transition-all duration-200",
                    active ? "border-[#3D0A14] text-[#3D0A14]" : completed ? "border-transparent text-[#3D0A14]/60" : "border-transparent text-[#9E7B7B]")}>
          ...
          <div className="relative h-56 md:h-72 bg-[#1A0A0A] overflow-hidden">
            <Image src={previewImage} alt={product.name} fill sizes="100vw" className="object-cover opacity-80 transition-opacity duration-500" />
            <div className="absolute inset-0 bg-gradient-to-t from-[#1A0A0A]/70 via-[#1A0A0A]/10 to-transparent" />
            <div className="absolute bottom-4 left-4">
              <span className="text-white/60 text-xs font-semibold uppercase tracking-widest">Step {step + 1} of {steps.length}</span>
              <h2 className="font-playfair text-white text-2xl font-bold leading-tight">{steps[step].label}</h2>
            </div>
          </div>
      ...
      <footer className="fixed bottom-0 left-0 right-0 z-50 bg-white/97 backdrop-blur-md border-t border-[rgba(26,10,10,0.10)] shadow-warm-sm p-4 md:pl-24">
        <button onClick={handleNext} disabled={!canProceed}
          className={cn("w-full font-bold py-4 rounded-2xl transition-all duration-300 font-playfair text-base tracking-wide",
            canProceed ? "bg-[#3D0A14] hover:bg-[#2D0810] text-white hover:shadow-warm-lg shadow-warm-sm active:scale-[0.97]" : "bg-[#3D0A14]/30 text-white/60 cursor-not-allowed")}>
